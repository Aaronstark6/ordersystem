...
# 当前小步：STAGE3_IMAGE_DETECTION_V1_01

目标：
- 在 Image Logic 中实现图片占位文字识别。
- 通过 analyzer 接入 `detect_images`，写入 `TemplateAnalysisResult.images`。
- 复用现有 Image 主链生成 `insert_image` operation。

边界：
- analyzer.py 只做调度接入，不写图片识别规则。
- 不修改 Field / Choice / Table / Condition。
- 不修改 DocumentModel / Workspace / Confirmed / Export / Executor。
# 当前小步：STAGE3_TABLE_DETECTION_GUARDRAIL_01

目标：
- 为 Table Detector V1 增加 guardrail。
- 避免字段布局、choice 行、image placeholder 行和 validation/control mapping 行被识别为 table。
- 保留真正表格表头：需要数据行或明确 table 标题证据。

边界：
- 不修改 analyzer 调度。
- 不修改 table operation contract。
- 不修改 ExportStrategy / Excel Executor。
# 当前小步：STAGE3_CONDITION_DETECTION_V1_01

目标：
- 在 Condition Logic 中实现简单条件文本识别。
- 通过 analyzer 接入 `detect_conditions`，写入 `TemplateAnalysisResult.conditions`。
- 复用现有 Condition Pipeline。

边界：
- analyzer.py 只做调度接入，不写条件识别规则。
