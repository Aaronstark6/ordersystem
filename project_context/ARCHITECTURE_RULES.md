...
- 仓库根目录禁止生成运行文件、缓存、导出文件和日志。
- `audit_output/` 只用于开发审计临时输出，不属于系统运行存储体系，不得被业务运行逻辑使用。
- 审计输出只能写入 `audit_output/`，并由 Owner 定期手动清理。
- 未来业务模块必须通过 Storage 规则获得路径，不得自行拼接路径写文件。
- 业务模块禁止直接拼接 `data/` 路径。
- 所有运行写入必须通过 Storage Layer，或明确遵守 Storage Layer 路径规则。
- `audit_output/` 不属于系统 Storage。
- `app/storage/` 只负责路径管理和授权目录创建，不负责业务判断。
- 任何系统运行代码中的 `open(..., "w")`、`save`、`export`、`mkdir` 或缓存写入逻辑，必须先验证解析后的目标路径位于 `data/`。
